...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			if (ClientPrefs.freeplayUnlocked.indexOf(song[0]) != -1) {
				var colors:Array<Int> = song[2];
				if(colors == null || colors.length < 3)
				{
					colors = [146, 113, 253];
		...
		bg = new FlxSprite().loadGraphic(Paths.image('backgroundmenu/menuDesat'));
		...
		var swag:Alphabet = new Alphabet(1, 0, "swag", true);
