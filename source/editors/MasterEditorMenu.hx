...
class MasterEditorMenu extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgroundmenu/menuDesat'));
		bg.scrollFactor.set();
		bg.color = 0xFF353535;
		add(bg);
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			switch(options[curSelected]) {
				...
				case 'Week Editor':
					MusicBeatState.switchAnimatedState(new WeekEditorState());
				case 'Menu Character Editor':
					MusicBeatState.switchAnimatedState(new MenuCharacterEditorState());
