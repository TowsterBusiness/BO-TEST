...
class Note extends FlxSprite
{
		...
		if(noteData > -1 && noteType != value) {
			switch(value) {
				case 'Hurt Note':
					...
					reloadNote('HURT');
					noteSplashTexture = 'GhostNoteSplash';
					colorSwap.hue = 0;
	...
	public function reloadNote(?prefix:String = '', ?texture:String = '', ?suffix:String = '') {
		...
		if(texture.length < 1) {
			...
			if (mustPress && PlayState.SONG.playerArrowSkin != null) skin = PlayState.SONG.playerArrowSkin;
			if(skin == null || skin.length < 1 || !ClientPrefs.customArrows) {
				skin = 'NOTE_assets';
			}
		}
	...
	function loadNoteAnims() {
		...
		if (isSustainNote)
		{
			animation.addByPrefix('purpleholdend', 'purple hold end0');
