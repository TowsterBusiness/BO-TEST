...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false, ?isElse = false)
	{
		weeksList = [];
		weeksLoaded.clear();
		#if MODS_ALLOWED
			...
			for (j in 0...directories.length) {
				...
				if(!weeksLoaded.exists(sexList[i])) {
					...
					if(week != null) {
						...
						if(weekFile != null && (isStoryMode == null || isElse || (isStoryMode && !weekFile.hideStoryMode) || (!isStoryMode && !weekFile.hideFreeplay))) {
