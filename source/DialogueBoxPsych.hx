...
class DialogueBoxPsych extends FlxSpriteGroup
{
		...
		if(!dialogueEnded) {
			...
			if(PlayerSettings.player1.controls.ACCEPT) {
				if(!daText.finishedText) {
					...
					daText = new Alphabet(DEFAULT_TEXT_X, DEFAULT_TEXT_Y, textToType, true, true, 0.0, 0.7);
					add(daText);
	...
	function startNextDialog():Void
	{
		...
		daText = new Alphabet(DEFAULT_TEXT_X, DEFAULT_TEXT_Y, textToType, true, true, curDialogue.speed, 0.7);
		add(daText);
